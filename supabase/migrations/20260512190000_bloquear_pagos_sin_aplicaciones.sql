-- =====================================================================
-- hidrored — Bloquear pagos sin aplicaciones (anti "pago huérfano")
--
-- Antes: registrar_pago_completo aceptaba p_aplicaciones vacío. Si el
-- cliente no tenía facturas pendientes en el momento del cobro (p. ej.
-- el cobrador recibió el dinero antes de que se generara la factura del
-- mes), el pago quedaba registrado pero sin vincularse a ninguna factura
-- (pago huérfano). Cuando luego aparecía la factura, ésta nacía como
-- pendiente y no había manera desde la UI de aplicarle ese pago previo.
--
-- Ahora: si p_aplicaciones viene vacío o suma cero, el RPC falla con un
-- mensaje claro. La UI debe deshabilitar el botón "Registrar pago" en
-- ese mismo escenario para evitar que el usuario llegue al RPC.
--
-- Importante: esta migración solo previene casos futuros. Los pagos
-- huérfanos pre-existentes deben aplicarse manualmente con un script
-- ad-hoc (ver doc interna o pedir al equipo el SQL puntual).
--
...
begin
  ...
  -- Anti pago huérfano: exigir al menos una aplicación con monto > 0.
  -- Sin esto, un pago sin aplicaciones queda "suelto" y no hay forma
  -- desde la UI de aplicarlo a una factura futura.
  if v_suma_aplicaciones <= 0 then
    raise exception 'no se puede registrar un pago sin aplicarlo a una factura. '
      ...
      using errcode = 'P0001',
            hint    = 'Si el cliente aún no tiene facturas pendientes, '
                      'genera la factura del periodo y luego registra el pago.';
  end if;
